...
void run_game(int computer_number); //added function to list
...
int main(int argc, char *argv[])
{
    ...
    while (prompt == 'y')   
    {
	  ...
	  prompt = ask_to_play(played); //misspelled played
    }
...
void run_game(int computer_number)
{
    ...
    scanf("%d", &number); //Used %d to call an int

int correct=0; //declared correct variable
    while (!correct)  
    {
